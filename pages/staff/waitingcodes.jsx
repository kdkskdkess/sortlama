...
class App extends React.Component {
    ...
    return (
        <>
            <Head>
                <title>{cfg.siteName} - {Lang("staff.sidebar.waitingCodes", this.props.lang)}</title>
            </Head>
            <ToastContainer />
            <div className="min-h-screen flex flex-row flex-col flex-auto flex-shrink-0 antialiased bg-gray-50 text-gray-800">
                ...
                <div className="bg-gray-900 h-screen w-screen px-16 md:px-64 p-6">
                    <div className="container mx-auto px-4 sm:px-8">
                        <div className="py-8">
                            <div>
                                <h2 className="text-2xl font-semibold leading-tight text-gray-200">{Lang("staff.waitingcodes.head", this.props.lang)}</h2>
                            </div>
                                        ...
                                        <thead>
                                            <tr>
                                                <th
                                                    ...
                                                    {Lang("staff.waitingcodes.name", this.props.lang)}
                                                </th>
                                                <th
                                                    className="px-5 py-3 bg-input text-left text-xs font-semibold text-gray-200 uppercase tracking-wider">
                                                    {Lang("staff.waitingcodes.owner", this.props.lang)}
                                                </th>
                                                <th
                                                    className="px-5 py-3 bg-input text-left text-xs font-semibold text-gray-200 uppercase tracking-wider">
                                                    {Lang("staff.waitingcodes.acts", this.props.lang)}
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {this.state.codes ?
                                            this.state.codes.sort(function(a, b) { return (b.views || 0) - (a.views || 0) }).map((code, i) => {
                                            return (<tr id={`code-${i}`}>
                                                    ...
                                                    <div className="flex">
                                                        <div className="badge cursor-pointer">
                                                            <span className="px-2 ml-auto text-xs font-medium tracking-wide text-red-800 bg-blue-100 rounded-full" onClick={async(e) => {
                                                                ...
                                                                if(d.error === 3) {
                                                                    return toast.error(Lang("staff.waitingcodes.regularly", this.props.lang), { position: "top-right", autoClose: 5000, hideProgressBar: false, closeOnClick: true, draggable: true, progress: undefined, });
                                                                }
                                                                if(d.error === 5) {
                                                                    return toast.error(Lang("staff.waitingcodes.youDontHavePermission", this.props.lang), { position: "top-right", autoClose: 5000, hideProgressBar: false, closeOnClick: true, draggable: true, progress: undefined, });
                                                                }
                                                                toast.success(Lang("staff.waitingcodes.deleted", this.props.lang), { position: "top-right", autoClose: 5000, hideProgressBar: false, closeOnClick: true, draggable: true, progress: undefined, });
                                                                document.getElementById(`code-${i}`).remove();
                                                                this.setState({codes: this.props.codes.filter((codee) => codee.name !== code.name)})
                                                                this.props.codes = this.state.codes.filter((codee) => codee.name !== code.name)
                                                            }}>{Lang("staff.waitingcodes.delete", this.props.lang)}</span>
                                                        </div>
                                                        <div className="badge ml-2 cursor-pointer">
                                                            <span className="px-2 ml-auto text-xs font-medium tracking-wide text-purple-800 bg-blue-100 rounded-full" onClick={async(e) => {
                                                                var d = await axios(cfg.siteURL+"/api/codes/approve", {
                                                                    method: "POST",
                                                                    headers: {
                                                                        'Accept': 'application/json',
                                                                        'Content-Type': 'application/json'
                                                                ...
                                                                toast.success(Lang("staff.waitingcodes.approved", this.props.lang), { position: "top-right", autoClose: 5000, hideProgressBar: false, closeOnClick: true, draggable: true, progress: undefined, });
                                                                document.getElementById(`code-${i}`).remove();
                                                                this.setState({codes: this.props.codes.filter((codee) => codee.name !== code.name)})
                                                                this.props.codes = this.state.codes.filter((codee) => codee.name !== code.name)
                                                            }}>{Lang("staff.waitingcodes.approve", this.props.lang)}</span>
                                                        </div>
