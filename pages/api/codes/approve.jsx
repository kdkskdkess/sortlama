...
export default async (req, res) => {
    ...
    if (req.method === "POST") {
        ...
        if(data.staff !== "CONSOLE") {
            ...
            if(codeSettings.publisherRoles) {
                if(codeSettings.publisherRoles.some(r=> staff.roles.includes(r)) === false) {
                    return res.json({error: 5})
                }
            }
        }
        await Codes.updateOne({ name: data.code.name }, {waiting: false});
        await TotalApprovedBotsAndCodes.updateOne({ id: data.staff }, { $inc: { codes: 1 } }, {upsert: true})

        sendLog(`Code "**${data.code.name}**" has been approved by <@${data.staff}>`)
